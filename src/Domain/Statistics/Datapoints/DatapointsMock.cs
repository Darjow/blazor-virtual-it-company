...
namespace Domain.Statistics.Datapoints
{
    ...
    public class DatapointsMock : Faker<DataPoint>
    {

        //singleton zodat hij niet steeds een nieuwe faker object moet creëren per virtual machine (memory heap)
        private static DatapointsMock _instance;

        public static DatapointsMock Instance
        {
            get
            {
                if (_instance is null)
                {
                    _instance = new DatapointsMock();
                }
                return _instance;
            }
        }
